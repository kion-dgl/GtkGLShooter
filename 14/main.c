...
static void on_realize(GtkGLArea *area) {
	...
	GLfloat enemy_bullets[] = {
		-enemies.bullet_radius, -enemies.bullet_radius,
		-enemies.bullet_radius, enemies.bullet_radius,
		enemies.bullet_radius, enemies.bullet_radius,
	...
	glGenBuffers(1, &enemies.bullet_vbo);
	glBindBuffer(GL_ARRAY_BUFFER, enemies.bullet_vbo);
	glBufferData(
		GL_ARRAY_BUFFER,
		sizeof(enemy_bullets),
		enemy_bullets,
		GL_STATIC_DRAW
	);
